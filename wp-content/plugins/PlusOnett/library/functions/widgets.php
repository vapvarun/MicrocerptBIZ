...
class WP_Widget_Custom_Taxonomy extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$cat_args = array('orderby' => 'name', 'show_count' => $c, 'hierarchical' => $h, 'taxonomy'=>$tn , 'id'=>$instance['taxonomy']   );
		if ( $d ) {
			$cat_args['show_option_none'] = __('Select Category');
			$a = get_categories(apply_filters('widget_categories_dropdown_args', $cat_args));			
?>
<select id="<?php echo $this->get_field_id('taxonomy'); ?>" onchange="onCatChange('<?php echo $this->get_field_id('taxonomy'); ?>','<?php echo $instance['taxonomy'];?>')" name="<?php echo $this->get_field_id('taxonomy'); ?>">
	<option value="0" >Select Category</option>
	<?php
		for($i=0;$i<count($a);$i++)
		{?>
    		<option value="<?php echo $a[$i]->term_id ?>" ><?php echo $a[$i]->name ?></option>
     <?php } ?>
</select>
<input type="hidden" name="category_count" id="category_count" value="<?php echo $c;?>" />
<input type="hidden" name="<?php echo $this->get_field_id('taxonomy'); ?>" id="<?php echo $this->get_field_id('taxonomy'); ?>" value="<?php echo $this->get_field_id('taxonomy'); ?>" />
...
/* <![CDATA[ */
	...
	//var t = '<?php //echo $this->get_field_id('taxonomy'); ?>'
	
	//var dropdown = document.getElementById(tn);
	
	function trim(stringToTrim) {
	return stringToTrim.replace(/^\s+|\s+$/g,"");
	}
	
	function onCatChange(a,b) {var tn = b;
		var w = document.getElementById(a).selectedIndex;
		
		//alert(document.getElementById(a).options[w].text);
		//if ( dropdown.options[dropdown.selectedIndex].value > 0) {	
			if (tn == 'category' ){
				
			location.href = "<?php echo home_url(); ?>/?cat="+document.getElementById(a).value;
			}
			else if (tn == 'post_tag' ) { 
				if(document.getElementById("category_count").value == "" || document.getElementById("category_count").value == 0)
				{
					var str = document.getElementById(a).options[w].text;
