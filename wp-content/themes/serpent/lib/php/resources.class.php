<?php
    class resources{
        ...
        static function  CallBox( $name , $args = null ) {
            ...
            if( isset( $_POST[ 'slidesettings' ] ) && isset( $_POST[ 'slidesettings' ][ 'height' ] ) && isset( $_POST[ 'post_ID' ] ) ){
                $postID = $_POST[ 'post_ID' ];
                $meta = meta::get_meta( $postID, 'slidesettings' );
                if( is_array( $meta ) ){
                    $oldheight = $meta[ 'height' ];
                    $newheight = $_POST[ 'slidesettings' ][ 'height' ];
                    if( $oldheight != $newheight ){
                        $slideshow_settings = $_POST[ 'slidesettings' ];
                        $data = meta::get_meta( $postID , 'box' );
                        foreach( $data as $slide ){
                            $res_type = $slide[ 'type_res' ];
                            $imageID = $slide[ 'slide_id' ];
                            $postID = $slide[ 'resources' ];

                            if( isset( $slideshow_settings[ 'height' ] ) && is_numeric( $slideshow_settings[ 'height' ] ) ){
                                $height = $slideshow_settings[ 'height' ];
                            }else{
                                $height = 300;
                            }

                            add_image_size( 'slideshow' , 990 , $height , true );
                            add_image_size( 'slideshow_large' , 1230 , $height , true );
                            ...
                            if( isset( $imageID ) && is_numeric( $imageID ) ){
                                ...
                                if( $path !== false && file_exists( $path ) ){
                                    $metadata = wp_generate_attachment_metadata( $imageID , $path );
